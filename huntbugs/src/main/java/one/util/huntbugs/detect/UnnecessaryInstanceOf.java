...
public class UnnecessaryInstanceOf {
    ...
    public void visit(Expression node, MethodContext mc, TypeDefinition td) {
        if (node.getCode() == AstCode.InstanceOf) {
            ...
            if (ynm == YesNoMaybe.YES) {
                report(mc, typeRef, expr, mc.findDeadCode(node, EdgeType.FALSE), eType.shrinkConstraint(typeRef, false),
                ...
                if (expr.getOperand() instanceof Variable && ((Variable) expr.getOperand()).getName().startsWith("jjte")
                    ...
                    return;
                report(mc, typeRef, expr, mc.findDeadCode(node, EdgeType.TRUE), eType.shrinkConstraint(typeRef, false),
            ...
            if (ynm == YesNoMaybe.NO) {
                report(mc, typeRef, expr, mc.findDeadCode(node, EdgeType.PASS), eType.shrinkConstraint(typeRef, false), "ImpossibleCast");
            }
            ...
            Nodes.ifBinaryWithConst(node, (arg, cst) -> {
                if (cst instanceof TypeReference) {
                    ...
                    if (arg.getCode() == AstCode.InvokeVirtual && Methods.isGetClass((MethodReference) arg
                        ...
                        if (ynm == YesNoMaybe.NO) {
                            report(mc, typeRef, obj, mc.findDeadCode(node, EdgeType.TRUE), eType.shrinkConstraint(
                                typeRef, true), "ClassComparisonFalse");
                        }
                    }
                }
            });
    ...
    private void report(MethodContext mc, TypeReference typeRef, Expression expr, CodeBlock deadCode, EType eType,
            String wType) {
        if (deadCode != null) {
            int priority = deadCode.isExceptional ? 40 : 0;
            mc.report(wType, priority, expr, Roles.TARGET_TYPE.create(typeRef), ETYPE.create(eType.toString()),
                Roles.EXPRESSION.create(expr), Roles.DEAD_CODE_LOCATION.create(mc, deadCode.startExpr));
        } else {
            mc.report(wType, 20, expr, Roles.TARGET_TYPE.create(typeRef), ETYPE.create(eType.toString()),
