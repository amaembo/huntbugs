...
public class UnnecessaryInstanceOf {
    ...
    public void visit(Expression node, MethodContext mc, TypeDefinition td) {
        if (node.getCode() == AstCode.InstanceOf) {
            ...
            if (ynm == YesNoMaybe.YES) {
                report(mc, typeRef, expr, eType.shrinkConstraint(typeRef, false), "UnnecessaryInstanceOf");
                ...
                if(expr.getOperand() instanceof Variable && ((Variable)expr.getOperand()).getName().startsWith("jjte") &&
                    ...
                    return;
                report(mc, typeRef, expr, eType.shrinkConstraint(typeRef, false), "ImpossibleInstanceOf");
            }
            ...
            if (ynm == YesNoMaybe.NO) {
                report(mc, typeRef, expr, eType.shrinkConstraint(typeRef, false), "ImpossibleCast");
            }
            ...
            Nodes.ifBinaryWithConst(node, (arg, cst) -> {
                if(cst instanceof TypeReference) {
                    ...
                    if(arg.getCode() == AstCode.InvokeVirtual && Methods.isGetClass((MethodReference) arg.getOperand())) {
                        ...
                        if(ynm == YesNoMaybe.NO) {
                            report(mc, typeRef, obj, eType.shrinkConstraint(typeRef, true), "ClassComparisonFalse");
                        }
                    }
                }
            });
    ...
    private void report(MethodContext mc, TypeReference typeRef, Expression expr, EType eType, String wType) {
        mc.report(wType, 0, expr, Roles.TARGET_TYPE.create(typeRef), ETYPE.create(eType.toString()), Roles.EXPRESSION
                .create(expr));
    }
}
